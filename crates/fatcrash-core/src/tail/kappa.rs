...
pub fn estimate_mad_of_sums(data: &[f64], block_size: usize) -> f64 {
    let n = data.len();
...
pub fn compute_taleb_kappa(data: &[f64], n0: usize, n: usize) -> f64 {
    if n <= n0 || n0 == 0 {
        return f64::NAN;
    }
    ...
    2.0 - log_ratio_n / log_ratio_m
}

/// Gaussian benchmark for Taleb kappa via Monte Carlo.
pub fn gaussian_taleb_kappa_benchmark(data_len: usize, n0: usize, n: usize, n_sims: usize) -> f64 {
...
pub fn taleb_kappa_slice(data: &[f64], n0: usize, n1: usize, n_sims: usize) -> (f64, f64) {
    let n1_val = n1.min(data.len() / 2);
...
pub fn compute_kappa(data: &[f64], n_subsamples: usize) -> f64 {
    let n = data.len();
...
/// Returns (kappa, gaussian_benchmark).
pub fn kappa_metric_slice(data: &[f64], n_subsamples: usize, n_sims: usize) -> (f64, f64) {
    let kappa = compute_kappa(data, n_subsamples);
    let benchmark = gaussian_kappa_benchmark(data.len(), n_subsamples, n_sims);
