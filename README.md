...
fatcrash detects crashes via fat-tail statistics. Python + Rust (PyO3). 8 methods, 112 tests, 500 years of data.
...
LPPLS detects all 39 drawdowns. Taleb kappa is most useful on major crashes (50%) where the tail structure genuinely deteriorates. Hill alpha alone is unreliable (28%) but contributes to the aggregate.
...
When 3+ independent method categories (bubble, tail, regime) agree, the probability gets a +15% bonus. At HIGH or CRITICAL, the combined detector is almost always right.
...
Every pair has Hurst H > 0.55 (universal persistence). 9/12 show explosive bubble episodes. Mean Taleb kappa 0.27 vs Gaussian benchmark 0.13 — the CLT converges at roughly half the expected rate.
...
19/30 countries have alpha < 2 (infinite variance). 25/30 have Hurst > 0.5 (persistent). Germany, Austria, Argentina, and Portugal saturate at Taleb kappa = 1.0 — Cauchy-like behavior where the CLT does not operate at any practical sample size.

Italy (H=0.80) and Portugal (H=0.85) show the strongest persistence over century-scale data.
...
make test                    # 20 Rust + 92 Python = 112 tests
