...
pub fn compute_spectral(data: &[f64], bandwidth_exp: f64) -> f64 {
    let n = data.len();
    if n < 32 {
        return f64::NAN;
    }
...
pub fn spectral_exponent_slice(data: &[f64], bandwidth_exp: f64) -> f64 {
    compute_spectral(data, bandwidth_exp)
}
