...
pub fn compute_bipower_variation(returns: &[f64]) -> f64 {
    ...
    // BV = (252 / (mu_1^2 * (W-1))) * sum(|r_i| * |r_{i-1}|)
    let sum: f64 = returns
        .windows(2)
...
pub fn compute_tripower_quarticity(returns: &[f64]) -> f64 {
    ...
    // For standard normal: E[|Z|^{4/3}] = 2^{2/3} * Gamma(7/6) / sqrt(pi)
...
pub fn compute_jump_test(returns: &[f64]) -> (f64, f64) {
    ...
    // Var = (pi^2/4 + pi - 5) * (1/n) * max(1, TPQ/BV^2)
...
pub fn bipower_variation_slice(returns: &[f64], window: usize) -> f64 {
    let n = returns.len();
    if window < 2 || window > n {
        return f64::NAN;
    }
    compute_bipower_variation(&returns[(n - window)..])
}
...
///     returns: Slice of log returns.
///     window: Window size (default 21).
pub fn jump_variance_slice(returns: &[f64], window: usize) -> f64 {
    let n = returns.len();
    if window < 2 || window > n {
        return f64::NAN;
    }
    compute_jump_variance(&returns[(n - window)..])
}
...
pub fn jump_test_slice(returns: &[f64], window: usize) -> (f64, f64) {
    let n = returns.len();
    if window < 3 || window > n {
        return (f64::NAN, f64::NAN);
    }
    compute_jump_test(&returns[(n - window)..])
}

/// Rolling bipower variation.
pub fn bipower_variation_rolling_slice(returns: &[f64], window: usize, step: usize) -> Vec<f64> {
    let n = returns.len();
    let step = step.max(1);
...
pub fn jump_variance_rolling_slice(returns: &[f64], window: usize, step: usize) -> Vec<f64> {
    let n = returns.len();
    let step = step.max(1);
