...
/// Returns (mu, sigma, xi) -- location, scale, shape.
/// Returns an error if fewer than 10 observations are provided.
pub fn gev_fit_slice(data: &[f64]) -> Result<(f64, f64, f64), String> {
    if data.len() < 10 {
        return Err("Need at least 10 observations for GEV fitting".to_string());
    }
    Ok(gev_fit_mle(data))
}
