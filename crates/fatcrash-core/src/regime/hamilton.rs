...
fn smooth_full(
    ...
    for t in (0..(n - 1)).rev() {
        ...
        // P(s_t=1 | all data) = sum_j P(s_t=1, s_{t+1}=j | all data)
        // = sum_j P(s_t=1 | y_{1..t}) * P(s_{t+1}=j | s_t=1) * P(s_{t+1}=j | all data) / P(s_{t+1}=j | y_{1..t})
...
pub fn hamilton_filter_slice(
    ...
    forward_filter(data, mu, sigma, p00, p11)
}
...
pub fn hamilton_fit_slice(
    ...
    let n_restarts = n_restarts.unwrap_or(10).max(1);
